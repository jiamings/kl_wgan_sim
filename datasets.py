...
def rings_log_pdf_grad(X, sigma=0.1, radia=np.array([1, 3])):
    ...
    for i in range(len(X)):
        ...
        grad_1d = np.dot(gs_inner.T, np.exp(
            neg_log_neg_ratios + np.log(weights)))
...
def rings_log_pdf(X, sigma=0.1, radia=np.array([1, 3])):
    ...
    if X.shape[1] > 2:
        ...
        result += np.sum(-0.5*np.log(2*np.pi*sigma**2) -
                         0.5 * (X[:, 2:]**2) / (sigma ** 2), 1)
...
class Spiral(ToyDataset):
    ...
    def sample(self, n):
        ...
        for si, s in enumerate(self.starts):
            ...
            data[si*batch_size:(si+1) *
                 batch_size] = self.sample_branch(batch_size, s)
        return data[:n, :]
    ...
    def _conditional_dpdf_one_dim(self, a, x, D):
        ...
        for si, s in enumerate(self.starts):
            ...
            m, s = self._branch_params_one(a, s)
            dpdf[si] = np.exp(norm.logpdf(
                x, loc=m, scale=s).sum()) * (- x[D] + m[D]) / s[D]**2
            dpdf[si] /= self.nstart
    ...
    def dpdf_one(self, x, *args, **kwargs):
        ...
        for d in range(self.D):
            dpdf[d] = quad(self._conditional_dpdf_one_dim,
                           0, 1, (x, d), *args, **kwargs)[0]
        return dpdf
...
class Funnel(ToyDataset):
    ...
    def grad_multiple(self, x):
        ...
        grad[:, 0] = -x1/self.sigma**2 - \
            (self.D-1)/2.0 * dlv - 0.5*(x[:, 1:]**2).sum(1) * (-dv)/v/v
...
class Cosine(ToyDataset):
    ...
    def grad_multiple(self, X):
        ...
        grad[:, 0] = -(X[:, 1]-m)/self.sigma**2 * self.A * \
            np.sin(self.omega*X[:, 0])*self.omega
...
class Banana(ToyDataset):
    ...
    def logpdf_multiple(self, x):
        ...
        logp = norm.logpdf(x[:, 0], 0, self.sigma) + \
            norm.logpdf(x[:, 1], self.bananicity *
                        (x[:, 0]**2-self.sigma**2), 1)
        if self.D > 2:
...
class RealDataset(Dataset):
    ...
    def stream(self, n, add_noise=False):
        ...
        d = np.take(self.data, range(
            self.pointer, self.pointer+n), mode="wrap", axis=0)
        if self.nkde:
            p = np.take(self.kde_logp, range(
                self.pointer, self.pointer+n), mode="wrap", axis=0)
        else:
            p = None
...
class WhiteWine(RealDataset):
    ...
    def __init__(self, *args, **kwargs):
        self.data = np.loadtxt("data/winequality-white.csv",
...
class RedWine(RealDataset):
    ...
    def __init__(self, *args, **kwargs):
        self.data = np.loadtxt("data/winequality-red.csv",
                               delimiter=";", skiprows=1)[:, :-1]
        self.name = "RedWine"
...
class HepMass(RealDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        train, test = self.load_data_no_discrete_normalised('data/')
        print(train.shape, test.shape)
        self.data = np.concatenate([train, test], axis=0)
        super(HepMass, self).__init__(ntest=174987, *args, **kwargs)

    def load_data(self, path):
        import pandas as pd
        from os.path import join
        data_train = pd.read_csv(filepath_or_buffer=join(
            path, "1000_train.csv"), index_col=False)
        data_test = pd.read_csv(filepath_or_buffer=join(
            path, "1000_test.csv"), index_col=False)
...
class Parkinsons(RealDataset):
    def __init__(self, cor=0.98, *args, **kwargs):
        self.data = np.loadtxt("data/parkinsons_updrs.data",
                               delimiter=",", skiprows=1)[:, 3:]
...
class Gas(RealDataset):
    def __init__(self, cor=0.98, *args, **kwargs):
        self.data = np.array(
            np.load("data/ethylene_CO.pickle", encoding='latin1'))[:, 3:]
        self.name = "Gas"
        self.data = clean_data(self.data, cor=cor)
...
def load_data(dname, noise_std=0.0, seed=1, D=None, data_args={}, **kwargs):
    ...
    if dname[0:2] == "re":
        ...
        p = RealToy(dname.title(), D=D, noise_std=0.0, ntest=1000,
                    seed=seed, data_args=data_args, **kwargs)
    return p
